...
partial class Program
{
    ...
    {
        var favoriteAncientWonder = WondersOfTheAncientWorld.StatueOfZeusAtOlymmpia;
    ...
    {
        Vocabularies.Default.AddIrregular("procurador geral", "procuradores gerais");
        Vocabularies.Default.AddIrregular("batata", "batatas");
        int number = 123;
        WriteLine($"Original number: {number}");
        WriteLine($"Roman: {number.ToRoman()}");
        WriteLine($"Words: {number.ToWords()}");
        WriteLine($"Original words: {number.ToOrdinalWords()}");
        WriteLine();
        // string[] things2 = { "fox", "person", "sheep", "apple", "goose",
        //     "oasis", "potato", "die", "dwarf", "attorney general", "biceps"};
        string[] things = { "raposa", "pessoa", "ovelha", "maça", "ganso",
            "oasis", "batata", "morte", "doende", "procurador geral", "biceps"};
        ...
        WriteLine("Original: {0}, Metric: {1}", thousands,
            thousands.ToMetric
                (MetricNumeralFormats.WithSpace | MetricNumeralFormats.UseShortScaleWord, decimals: 0));
    ...
    private static void DateTimeFormatting()
    {
        ...
        WriteLine("Add 7 days, Humanize: {0}", now.AddDays(7).Humanize());
        WriteLine("Add 7 days and 1 minute, Humanized: {0}", now.AddMonths(7).AddMinutes(1).Humanize());
        WriteLine("Add 1 month, Humanized: {0}", now.AddMonths(1).Humanize());
        WriteLine();

        //Examples fo TimeSpan humanization.
        ...
        TimeOnly[] times = {
                new TimeOnly(9, 0), new TimeOnly(9, 15), new TimeOnly(15, 30)};
