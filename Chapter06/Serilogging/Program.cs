...
// a text file, one-file-per-day, name with the date
using Logger log = new LoggerConfiguration().WriteTo.Console().
    WriteTo.File("log.txt", rollingInterval: RollingInterval.Day).CreateLogger();

//Assign the new logger to the static to the entry point for logging.
Log.Logger = log;
